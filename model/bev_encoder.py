...
class BevModel(nn.Module):
    def __init__(self, model_cfg, in_channel=3):
        ...
        self.conv1 = Conv1dBN(in_channel, 128)
        self.backbone = timm.create_model('vit_small_patch8_224.dino', pretrained=True, features_only=True)
        ...
        self.shared_fc = Conv1dBN(384+128, 128)
    ...
    def forward(self, batch_dict):
        ...
        bev = batch_dict['bev_image']
        inverse_mapping = batch_dict['inverse_mapping']
        if self.training:
            vectors = batch_dict['vectors']
            offset, cls = self.assign_targets(xyz[:, :, :3], vectors, self.model_cfg.PosRadius)
        ...
        feats = self.conv1(l0_fea) # (B, 128, N)

        B, H, W, C = bev.shape

        bev = bev.permute(0, 3, 1, 2)
        im_feat, _, _ = self.backbone(bev) # all three layers
        im_feat = torch.nn.functional.interpolate(im_feat, size=(H, W), mode='bilinear', align_corners=False)
        # im_feat2 = torch.nn.functional.interpolate(img_feat2, size=(H, W), mode='bilinear', align_corners=False)
        # im_feat3 = torch.nn.functional.interpolate(img_feat3, size=(H, W), mode='bilinear', align_corners=False)
        im_feat = im_feat.permute(0, 2, 3, 1)
        # im_feat2 = im_feat2.permute(0, 2, 3, 1)
        # im_feat3 = im_feat3.permute(0, 2, 3, 1)
        p_feat  = []
        for i in range(B):
            p_feat.append(im_feat[i, inverse_mapping[i, 1, :], inverse_mapping[i, 0, :]])
            # p_feat2.append(im_feat2[i, inverse_mapping[i, 1, :], inverse_mapping[i, 0, :]])
            # p_feat3.append(im_feat3[i, inverse_mapping[i, 1, :], inverse_mapping[i, 0, :]])
        p_feat = torch.stack(p_feat, dim=0)
        p_feat = self.drop_path(p_feat.permute(0, 2, 1))

        p_feat = torch.cat([p_feat, feats], dim=1)
