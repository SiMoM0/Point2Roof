...
class EdgeAttentionNet(nn.Module):
    def __init__(self, model_cfg, input_channel):
        ...
        self.att_layer = EdgeModule(input_channel)
...
class Block(nn.Module):
    ...
    def forward(self, x1, x2):
        x = x1 + self.drop_path(self.attn(self.norm1(x1), self.norm2(x2)))
        x = x + self.drop_path(self.mlp(self.norm3(x)))

        return x

class EdgeModule(nn.Module):
    def __init__(self, input_channel, block=True, drop_path=0.1):
        super().__init__()
        self.input_channel = input_channel
        self.att1 = Block(input_channel, 4, drop_path=drop_path) if block else Attention(input_channel)
        self.att2 = Block(input_channel, 4, drop_path=drop_path) if block else Attention(input_channel)
        self.num_output_feature = input_channel
        self.pool = nn.MaxPool1d(2)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

    def forward(self, point_fea1, point_fea2):
        fea1 = point_fea1 + self.drop_path(self.att1(point_fea1, point_fea2))
        fea2 = point_fea2 + self.drop_path(self.att2(point_fea2, point_fea1))
        # max pooling
        fea = torch.cat([fea1.unsqueeze(1), fea2.unsqueeze(1)], 1)
        fea = self.pool(fea.permute(0, 2, 1)).squeeze(-1)
        return fea
